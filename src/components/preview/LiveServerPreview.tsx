...
export const LiveServerPreview: React.FC = () => {
  const { files, activeFileId, theme } = useIDEStore();
  ...
  // Find active HTML file or first HTML file in workspace
  const htmlFile =
    files.find((f) => f.language === 'html' || f.name.endsWith('.html')) ||
    files.find((f) => f.id === activeFileId) ||
    files[0];

  // Sync workspace files to live server backend whenever files update
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          <span className="flex items-center gap-1.5 text-[11px] font-semibold text-[#23d18b]">
            <span className="w-2 h-2 rounded-full bg-[#23d18b] inline-block animate-pulse" />
          ...
          >
            ...
            {copiedUrl && <span className="text-[10px] text-[#23d18b] ml-1">Copied!</span>}
          </div>
        </div>
        ...
        <div
          className={`flex items-center rounded border p-0.5 ${
            isTurboTheme ? 'bg-[#000055] border-[#55FFFF]' : 'bg-[#181818] border-[#2e2e2e]'
          }`}
        >
          <button
            type="button"
      ...
      {/* Frame Container */}
      <div className="flex-1 w-full overflow-auto bg-[#0a0a0a] flex items-center justify-center p-2">
        <div
          style={{ width: containerWidth, maxWidth: '100%', height: '100%' }}
          className="bg-white rounded-md shadow-2xl overflow-hidden border border-[#333333] transition-all duration-200"
        >
          <iframe
            key={key}
            ref={iframeRef}
